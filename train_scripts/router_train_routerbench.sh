    ...
    for top_k in "${top_ks[@]}"; do
        for last_k in "${last_ks[@]}"; do
            for seed in "${seeds[@]}"; do
                python -u train_router_mdeberta_routerbench.py --training_steps ${training_steps} --top_k ${top_k} --last_k ${last_k} --learning_rate ${learning_rate} --eval_steps 100 --tempreture ${tempreture} --similarity_function ${similarity_function} --sample_loss_weight ${sample_loss_weight} --regular_loss_weight ${regular_loss_weight} --cluster_loss_weight ${cluster_loss_weight} --seed ${seed} --final_eval --cost_rate ${cost_rate} \
